  ...
  activate() {
    ...
    const commandsDisposable = atom.commands.add('atom-text-editor', {
      'pulsar-idris-lsp:evaluate-in-repl': async () => {
        ...
        const decoratorBuffer = new TextBuffer({text: result})
        decoratorEditor = atom.workspace.buildTextEditor({
          mini: true, readOnly: true, buffer: decoratorBuffer
        })
        if (isSuccess) {
        ...
        element.onDidAttach(() => {
          decoratorEditor.setEditorWidthInChars(40)
          element.measureDimensions()
          element.setWidth(decoratorEditor.getEditorWidthInChars() * element.getDefaultCharacterWidth())
          element.setHeight(element.component.getLineHeight())
        })
        decoration = editor.decorateMarker(marker, {type: 'overlay', item: element})
      }
    })
