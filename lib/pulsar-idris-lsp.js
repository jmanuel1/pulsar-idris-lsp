...
const { symbolInformationToString } = require('./symbol-information')
...
class IdrisLanguageClient extends AutoLanguageClient {
  ...
  activate() {
    ...
    const commandsDisposable = atom.commands.add('atom-text-editor', {
      ...
      // https://github.com/idris-community/idris2-nvim/blob/bd282b74068e53e94d0c40ccc52f59eed3be909a/lua/idris2/browse.lua#L92
      'pulsar-idris-lsp:browse-namespace': () => {
        // FIXME: Blatant accessibility error. Use form elements. I'm pretty
        // sure I copied this pattern from the command palette.
        const editor = atom.workspace.buildTextEditor({mini: true})
        const helpMessage = document.createElement('p')
        helpMessage.textContent = 'Enter namespace'
        const modalContent = document.createElement('div')
        modalContent.appendChild(editor.element)
        modalContent.appendChild(helpMessage)
        ...
        const editorCommandsDisposable = atom.commands.add(editor.element, {
          'core:confirm': async () => {
            const namespace = editor.getText().trim()
            const codeActionParams = {
              textDocument: Convert.editorToTextDocumentIdentifier(activeEditor),
              context: {
                diagnostics: [],
            ...
            const selectList = new SelectListView({
              items: [],
              elementForItem({ label }) {
                const element = document.createElement('li')
                element.textContent = label
                return element
              },
              // TODO: Improve filtering by doing fuzzy search on each part of
              // the SymbolInformation separately.
              filterKeyForItem({ label }) {
                return label
              },
              emptyMessage: `No names found in ${namespace}. Maybe it's not imported?`,
              loadingMessage: 'Loading contents of namespace ' + namespace,
              didConfirmSelection: async ({ symbolInformation: { location } }) => {
                const path = Convert.uriToPath(location.uri)
                const position = Convert.positionToPoint(location.range.start)
                await atom.workspace.open(path, {initialLine: position.row, initialColumn: position.column})
                close()
            ...
            const labelledResults = (result || []).map(symbolInformation => ({
              symbolInformation,
              label: symbolInformationToString(symbolInformation)
            }))
            selectList.update({items: labelledResults, loadingMessage: null})
          },
          'core:cancel': () => {
            close()
          },
